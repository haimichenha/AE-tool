"""Version-pinned remaining sixteen lighting title relocation, isolated output only."""
...
SUPPORTED={'Drop_Shadow': 19, 'Glint_Rainbow': 38, 'Glow_Aura': 40, 'Glow_Aura_Autogen': 40, 'Glow_Darks': 30, 'Glow_Darks_Autogen': 30, 'Glow_Dist': 30, 'Glow_Dist_Autogen': 30, 'Glow_Edges': 34, 'Glow_Edges_Autogen': 34, 'Glow_Noise': 42, 'Glow_Noise_Autogen': 42, 'Glow_Orthicon': 28, 'Glow_Orthicon_Autogen': 28, 'Glow_Rainbow': 37, 'Glow_Rainbow_Autogen': 37, 'Glow_Rings': 36, 'Glow_Rings_Autogen': 36, 'LensFlare': 74, 'LensFlareAutoTrack': 73, 'LightLeak': 70, 'LightLeak_Autogen': 84, 'SpotLight': 34, 'SpotLight_Autogen': 47, 'UltraGlow': 71, 'Z_Glow': 29}
...
def build(source,mapping,output):
 ...
 report={'source_sha256':SOURCE_SHA,'output_sha256':sha(out),'mapping_sha256':sha(Path(mapping).read_bytes()),'effects':list(m['entries']),'definitions':records,'title_insertions':sum(x['parameter_count'] for x in records),'static_invariants':'only title insertions, 26 existing definition pointers and PE metadata; defaults, enums, expression bodies and existing executable bytes unchanged','runtime_validation':'pending','all_ui_translated':False}
