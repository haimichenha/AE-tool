"""Native Glare title-only candidate on the pinned known-good Light3D core.
...
def parameter_segments(definition):
 ...
 if end!=len(definition) or [n.atom(definition) for n in root.children[:2]]!=[b'def_effect',b'Glare']:raise ValueError('not a complete Glare definition')
...
def build(source,mapping,destination):
 ...
 if m['effect']!='Glare' or m['encoding']!='cp936':raise ValueError('mapping effect/encoding mismatch')
 titles=m['parameters'];needle=b'(def_effect Glare '
 if b.count(needle)!=1:raise ValueError('ambiguous Glare definition')
 ...
 if len(keys)!=36 or len(set(keys))!=36 or set(keys)!=set(titles):raise ValueError('mapping keys must exactly match all36 native parameters')
 edits=[]
 ...
 out[header:header+40]=struct.pack('<8sIIIIIIHHI',b'.spgl\0\0\0',len(body),va,rs,raw,0,0,0,0,0x40000040)
 ...
 report={'source_sha256':SOURCE_SHA,'output_sha256':sha(out),'effect':'Glare','mapped_titles':len(keys),'keys':keys,'definition_offset':hex(start),'definition_pointer_offset':hex(refs[0]),'section':'.spgl','mapping_sha256':sha(Path(mapping).read_bytes()),'static_check':'only Glare title insertions, one definition pointer and PE metadata; existing code and other definitions unchanged','runtime_validation':'pending','all_ui_translated':False,'unmapped_display_types':['buttons','groups','popup captions','dynamic shared UI']}
