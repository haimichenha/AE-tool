"""Version-pinned multi-effect native title relocation, isolated output only."""
...
SUPPORTED={'BokehLights':48,'EdgeRays':48,'Flashbulbs':16,'Flashbulbs_Autogen':27,'Glint':38,'Glow':39,'Glow_Autogen':39,'Streaks':34}
...
def build(source,mapping,output):
 ...
 report={'source_sha256':SOURCE_SHA,'output_sha256':sha(out),'mapping_sha256':sha(Path(mapping).read_bytes()),'effects':list(m['entries']),'definitions':records,'title_insertions':sum(x['parameter_count'] for x in records),'static_invariants':'only title insertions, eight existing definition pointers and PE metadata; defaults, enums, expression bodies and existing executable bytes unchanged','runtime_validation':'pending','all_ui_translated':False}
